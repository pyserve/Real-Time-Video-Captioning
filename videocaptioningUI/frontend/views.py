...
class VideoCaptionView(View):
    def __init__(self):
        ...
        self.img_model = Model(
            inputs=self.img_model.inputs, outputs=self.img_model.layers[-2].output
        )
        self.max_length = 35
    ...
    def post(self, request):
        ...
        for frame in processed_frames:
            ...
            generated_caption = self.predict_caption(
                self.model, feature, self.tokenizer, self.max_length
            )
            gen_captions.append(generated_caption)

        captions = (
            ". ".join(set(gen_captions)).replace("startseq", "").replace("endseq", "")
        )
    ...
    def process_uploaded_file(self, file_uploaded):
        ...
        if not isinstance(file_uploaded, (TemporaryUploadedFile, InMemoryUploadedFile)):
            raise ValueError(
                "Invalid file type. Expected TemporaryUploadedFile or InMemoryUploadedFile."
            )
    ...
    def process_video_frames(video_path, max_frames=10):
        ...
        selected_frame_indices = np.random.choice(
            len(frames), max_frames, replace=False
        )
